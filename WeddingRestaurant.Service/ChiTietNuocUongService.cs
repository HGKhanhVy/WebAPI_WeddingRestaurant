...
namespace WeddingRestaurant.Service
{
    ...
    public class ChiTietNuocUongService : Base.Service, IChiTietNuocUongService
    {
        ...
        {
            var entities = _ctncRepository.Get(_ => !_.TrangThai.Equals("Da xoa")).ToList();
            int count = 0;
        ...
        {
            if (_ctncRepository.Get(_ => _.MaNuoc.Equals(model.MaNuoc) && _.MaTiec.Equals(model.MaTiec) && !_.TrangThai.Equals("Da xoa")).Any())
            {
        ...
        {
            var entity = _ctncRepository.GetTracking(x => x.MaNuoc.Equals(id) && x.MaTiec.Equals(id_Tiec) && !x.TrangThai.Equals("Da xoa")).FirstOrDefault();
            if (entity == null)
            {
        ...
        {
            var entity = _ctncRepository.GetSingle(_ => _.MaNuoc.Equals(id) && _.MaTiec.Equals(id_Tiec) && !_.TrangThai.Equals("Da xoa"));
            return entity;
        }
        ...
        public Task UpdateAsync(string Id, string id_Tiec, ChiTietNuocUongModel model, CancellationToken cancellationToken = default)
        {
            ...
            {
                var isDuplicate = _ctncRepository.GetTracking(x => x.MaNuoc.Equals(model.MaNuoc) && x.MaTiec.Equals(model.MaTiec) && !x.TrangThai.Equals("Da xoa")).FirstOrDefault();
                if (isDuplicate != null)
                {
