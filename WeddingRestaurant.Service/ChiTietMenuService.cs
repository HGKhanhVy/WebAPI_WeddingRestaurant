...
namespace WeddingRestaurant.Service
{
    ...
    public class ChiTietMenuService : Base.Service, IChiTietMenuService
    {
        ...
        {
            var entities = _ctmenuRepository.Get(_ => !_.TrangThai.Equals("Da xoa")).ToList();
            int count = 0;
        ...
        {
            if (_ctmenuRepository.Get(_ => _.MaMenu.Equals(model.MaMenu) && _.MaTiec.Equals(model.MaTiec) && !_.TrangThai.Equals("Da xoa")).Any())
            {
        ...
        {
            var entity = _ctmenuRepository.GetTracking(x => x.MaMenu.Equals(id) && x.MaTiec.Equals(id_Tiec) && !x.TrangThai.Equals("Da xoa")).FirstOrDefault();
            if (entity == null)
            {
        ...
        {
            var entity = _ctmenuRepository.GetSingle(_ => _.MaMenu.Equals(id) && _.MaTiec.Equals(id_Tiec) && !_.TrangThai.Equals("Da xoa"));
            return entity;
        }
        ...
        public Task UpdateAsync(string Id, string id_Tiec, ChiTietMenuModel model, CancellationToken cancellationToken = default)
        {
            ...
            {
                var isDuplicate = _ctmenuRepository.GetTracking(x => x.MaMenu.Equals(model.MaMenu) && x.MaTiec.Equals(model.MaTiec) && !x.TrangThai.Equals("Da xoa")).FirstOrDefault();
                if (isDuplicate != null)
                {
