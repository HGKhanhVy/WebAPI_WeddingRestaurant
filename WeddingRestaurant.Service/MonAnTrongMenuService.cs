...
namespace WeddingRestaurant.Service
{
    ...
    public class MonAnTrongMenuService : Base.Service, IMonAnTrongMenuService
    {
        ...
        {
            if (_monMenuRepository.Get(_ => _.MaMonAn.Equals(model.MaMonAn) && _.MaMenu.Equals(model.MaMenu) && !_.TrangThai.Equals("Da xoa")).Any())
            {
        ...
        {
            var entity = _monMenuRepository.GetTracking(x => x.MaMonAn.Equals(id) && x.MaMenu.Equals(id_Menu) && !x.TrangThai.Equals("Da xoa")).FirstOrDefault();
            if (entity == null)
            {
        ...
        {
            var entities = _monMenuRepository.Get(_ => !_.TrangThai.Equals("Da xoa")).ToList();
            return (ICollection<MonAnTrongMenuEntity>)entities;
        }

        public MonAnTrongMenuEntity GetByKeyIdAsync(string id_Menu, string id)
        {
            var entity = _monMenuRepository.GetSingle(_ => _.MaMonAn.Equals(id) && _.MaMenu.Equals(id_Menu) && !_.TrangThai.Equals("Da xoa"));
            return entity;
        }

        public Task UpdateAsync(string Id, string id_Menu, MonAnTrongMenuModel model, CancellationToken cancellationToken = default)
        {
            var entity = _monMenuRepository.GetTracking(x => x.MaMonAn.Equals(Id) && x.MaMenu.Equals(Id) && !x.TrangThai.Equals("Da xoa")).FirstOrDefault();
            if (entity == null)
            {
                _logger.Information(ErrorCode.NotFound, Id + " - " + id_Menu);
                throw new CoreException(code: ResponseCodeConstants.NOT_FOUND, message: ReponseMessageConstantsMonAnTrongMenu.MONANTRONGMENU_NOT_FOUND, statusCode: StatusCodes.Status404NotFound);
            ...
            {
                var isDuplicate = _monMenuRepository.GetTracking(x => x.MaMonAn.Equals(model.MaMonAn) && x.MaMenu.Equals(model.MaMenu) && !x.TrangThai.Equals("Da xoa")).FirstOrDefault();
                if (isDuplicate != null)
                {
